...
const register=(req,res)=>{
    try {
        ...
        pool.query('SELECT * FROM public."HealthApp_lab" WHERE lab_id=$1',[lab_id],(err,response)=>{
            if(!!(!lab_id)){
                return res.status(500).json({success:"false",message:"lab_id is empty"})
            }
            if(!!(!lab_name)){
                return res.status(500).json({success:"false",message:"lab_name is empty"})
            }
            if(!!(!lab_address)){
                return res.status(500).json({success:"false",message:"Lab Address is empty"})
            }
            if(!!(!lab_phone)){
            ...
            if(err){
                ...
                return res.status(404).json({success:"false",message:`lab with lab_id ${lab_id} already exist`})
            }
            else{
                pool.query('INSERT INTO public."HealthApp_lab" (lab_id,lab_name,lab_address,lab_phone) VALUES($1,$2,$3,$4)',[lab_id,lab_name,lab_address,lab_phone],(err,response)=>{
                    if(err){
                        ...
                        return res.status(200).json({success:"true",lab_id:lab_id})
                    }
                })
            }
        })
